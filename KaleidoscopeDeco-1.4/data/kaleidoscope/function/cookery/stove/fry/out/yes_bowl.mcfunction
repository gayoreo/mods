#返还碗
execute if data block ~ ~ ~ Items[{Slot:15b,count:1}] run item replace block ~ ~ ~ container.15 with air
...
execute store result entity @e[type=item,tag=k_stove_item,limit=1] Item.count int 0.99 run data get entity @e[type=item,tag=k_stove_item,limit=1] Item.count
