...
execute positioned ~1 ~ ~ as @e[type=marker,distance=..0.1,tag=dc_pivot,tag=kaleidoscope_bar_coffee_table] store result score #temp kaleidoscope_temp run data get entity @s data.Rotation[0]
...
#switch
execute as @n[tag=dc_custom_display] run function kaleidoscope:bar/events/coffee_table/connect/_