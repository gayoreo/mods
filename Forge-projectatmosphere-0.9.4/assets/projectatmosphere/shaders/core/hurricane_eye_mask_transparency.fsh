...
uniform sampler2D SourceAccumSampler;
...
void main() {
    vec4 sourceAccum = texture(SourceAccumSampler, texCoord);
    float sourceRevealage = texture(SourceRevealageSampler, texCoord).r;
    float sourceDepth = texture(SourceDepthSampler, texCoord).r;

    if (ProtectionEnabled == 0 || StormCount <= 0) {
        accumColor = sourceAccum;
        revealage = sourceRevealage;
